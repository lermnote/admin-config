...
final class ToolFieldTypes {
	...
	private static function render_backup_tools_field( array $field ): void {
		$export_label = (string) ( $field['export_label'] ?? __( 'Export current settings', 'lerm-admin-config' ) );
		$import_label = (string) ( $field['import_label'] ?? __( 'Import settings JSON', 'lerm-admin-config' ) );
		$placeholder  = (string) ( $field['placeholder'] ?? __( '{ "example": "Paste a backup snapshot here" }', 'lerm-admin-config' ) );
		...
		echo '<button type="button" class="button button-secondary" data-lerm-backup-export>' . esc_html__( 'Generate snapshot', 'lerm-admin-config' ) . '</button>';
		echo '</div>';
		...
		echo '<button type="button" class="button button-primary" data-lerm-backup-import>' . esc_html__( 'Import snapshot', 'lerm-admin-config' ) . '</button>';
		echo '</div>';
